...
void ICACHE_FLASH_ATTR airkiss_app_upload_status()
{
    ...
    int ret = airkiss_json_upload(
        ...
        local_mcu_status.is_gas_detected,
		local_mcu_status.gas_value
		);
    if(ret == 0)
...
//    airkiss_app_msg_t airkiss_app_msg;
    ...
    my_cJSON *js_is_gas_detected = NULL;
    char *s = NULL;
    int ret = -1;
...
//            airkiss_app_msg.power_switch = (js_on_off->type == my_cJSON_False)?0:1;
		...
		*/
        ...
        if(os_strcmp(js_msg_type->valuestring, "set", os_strlen("set")) == 0)
        {
        	...
        	*/
        if(os_strcmp(js_msg_type->valuestring, "get", os_strlen("get")) == 0)
        {
            os_memset(airkiss_json_send_buffer, 0, sizeof(airkiss_json_send_buffer));
            ret = airkiss_json_ask_get(airkiss_json_send_buffer, js_msg_id->valueint, js_user->valuestring, local_mcu_status.status, local_mcu_status.is_gas_detected);
        }
