...
//TODO
int ICACHE_FLASH_ATTR airkiss_json_ask_get(char * _buf, uint64_t _msg_id, char *_user, uint8_t _operation_status, bool _is_gas_detected)
{
    my_cJSON *root;
    my_cJSON * js_services ;
    my_cJSON * js_operation_status ;
    my_cJSON * js_gas_detect ;
    /*create json string root*/
    root = my_cJSON_CreateObject();
    if(!root)
    {
    ...
    const char *const servers = "services";
    const char *const operation_status = "operation_status";
    const char *const gas_detect = "gas_detect";

    my_cJSON_AddItemToObject(root, servers, js_services=my_cJSON_CreateObject());
    my_cJSON_AddItemToObject(js_services, operation_status, js_operation_status=my_cJSON_CreateObject());
    my_cJSON_AddItemToObject(js_services, gas_detect, js_gas_detect=my_cJSON_CreateObject());
    ...
    if(_is_gas_detected == 0)
        {
            my_cJSON_AddFalseToObject(js_gas_detect, "is_gas_detected");
        }
        else
        {
            my_cJSON_AddTrueToObject(js_gas_detect, "is_gas_detected");
        }
...
int ICACHE_FLASH_ATTR airkiss_json_upload(char * _buf, uint8_t _operation_status, bool _is_gas_detected, uint16_t _gas_value)
{
    my_cJSON *root;
    my_cJSON * js_services ;
    my_cJSON * js_operation_status ;
    ...
    my_cJSON_AddStringToObject(root, "device_id", DEVICE_ID);
    my_cJSON_AddStringToObject(root, "device_type",DEVICE_TYPE);
    my_cJSON_AddStringToObject(root, "msg_type", "notify");
    my_cJSON_AddStringToObject(root, "data","gas_value");
